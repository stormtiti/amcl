...
void map_update_cspace(map_t *map, double max_occ_dist)
{
  ...
  for(int i=0; i<map->size_x; i++)
  {
    ...
    {
      if(map->cells[MAP_INDEX(map, i, j)].occ_state == +1) // occ dist
      {
...
//      else if (map->cells[MAP_INDEX(map, i, j)].occ_state == 0) { // unknown
//  	    map->cells[MAP_INDEX(map, i, j)].occ_dist = 5.0; // unknown penality
//      }
      else
