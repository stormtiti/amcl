...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  case ODOM_MODEL_DIFF:
  {
    ...
    for (int i = 0; i < set->sample_count; i++)
    {
      ...
      // Apply sampled update to particle pose
      sample->pose.v[0] += delta_trans * cos(sample->pose.v[2] + delta_rot1);
      sample->pose.v[1] += delta_trans * sin(sample->pose.v[2] + delta_rot1);
      sample->pose.v[2] += delta_rot1 + delta_rot2;
    }
  }
  break;
